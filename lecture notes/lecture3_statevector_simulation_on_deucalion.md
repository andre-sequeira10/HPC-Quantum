...
- [Lecture 3 - Quantum statevector simulation on Deucalion](#lecture-3---quantum-statevector-simulation-on-deucalion)
  - [1. Distributed statevector simulation on Deucalion's partitions ](#1-distributed-statevector-simulation-on-deucalions-partitions-)
    - [1.1 ARM partitions ](#11-arm-partitions-)
    - [1.2 x86 partitions ](#12-x86-partitions-)
    - [1.3 GPU partitions ](#13-gpu-partitions-)
  - [2. Slurm Basics ](#2-slurm-basics-)
    - [2.1 Create a batch script](#21-create-a-batch-script)
    - [2.2 Submit a batch job](#22-submit-a-batch-job)
    - [2.3 Examine the queue](#23-examine-the-queue)
    - [2.4 Cancel a job](#24-cancel-a-job)
    - [2.5 CPU Jobs](#25-cpu-jobs)
    - [2.6 GPU jobs](#26-gpu-jobs)
...
## 1. Distributed statevector simulation on Deucalion's partitions <a id="1-distributed-statevector-simulation-on-deucalions-partitions-"></a>
...
<div align="center">
  ...
  <p><em>Table 1: Standard publicly available partitions on Deucalion, with total RAM available for statevector simulation (assuming 16 bytes per amplitude)</em></p>
</div>

---

### 1.1 ARM partitions <a id="11-arm-partitions"></a>
...
<p><em>Table 2: Maximum qubits and required nodes for ARM partitions</em></p>
</div>
...
### 1.2 x86 partitions <a id="12-x86-partitions"></a>
...
<p><em>Table 3: Maximum qubits and required nodes for x86 partitions</em></p>
</div>
...
### 1.3 GPU partitions <a id="13-gpu-partitions"></a>
...
<p><em>Table 4: Maximum qubits and required GPUs/nodes for `normal-a100-40` GPU partition</em></p>
</div>
...
<p><em>Table 5: Maximum qubits and required GPUs/nodes for `normal-a100-80` GPU partition</em></p>
</div>


## 2. Slurm Basics <a id="2-slurm-basics-"></a>
...
Provided your account has access to a given partition you can set it in your batch script using the `#SBATCH --partition` directive, by setting the partition name as in Table 1. 
...
Each node on every GPU partition in Deucalion (see Table 1) is equipped with up to 4 GPUs, all of which can be used simultaneously within a single job. The following job script requests 4 GPUs, fully utilizing a single node:
...
1.	Parses --n_qubits (default 1).
2.	Builds a circuit: H on qubit 0, then CNOT(0→i) for i=1..n-1 → prepares GHZ_n.
3.	Creates QuantumState(n) (starts in |0…0\rangle), applies the circuit, and saves the time it takes to update the state.
4.	Prints the statevector and elapsed time.

⚠️ For large n (e.g., 28), printing the full vector is huge; it’s common to instead print only selected amplitudes (e.g., index 0 and 2^n\!-\!1) or summary stats.
...
3.	Uses a job array 4-28 → launches one run per n ∈ [4..28] (value passed via SLURM_ARRAY_TASK_ID).
...
1.	Save *.py and *.sh in the same directory.
2.	Submit the array job with `sbatch run_ghz.sh`.
