...
import os, psutil       # pip install psutil

_process = psutil.Process(os.getpid())

def mem_rss(msg=""):
    mbytes = _process.memory_info().rss / 1024**2   # bytes → MiB
    print(f"{msg:20s}  RSS = {mbytes:9.2f} MiB")
...
if __name__ == "__main__":
    ...
    #print(mem_rss("after state created"))   
    ...
    #print("DONE STATE")
    #print(mem_rss("after update")    )         

    # sampling using the state vector
    samples = st.sampling(10, seed)
    #s_zero = st.get_zero_probability(0)
    #a_zero = st.get_amplitude(0)

    #print(f"Zero ampltiude: {s_zero:.6f} (rank={mpirank})")
    #print(samples)
    if mpirank == 0:
